...
import { zasadyDoc, loDocs, internatDocs } from "@/lib/docs";
import { translations, languageOptions } from "@/lib/site-i18n";
import { GraduationCap, HeartHandshake, ShieldCheck, Trophy, Phone, Mail, MapPin, ArrowRight, Home, Utensils, Wifi, Languages, Check, FileText, ChevronDown, Download, Instagram, Facebook, Archive as ArchiveIcon, } from "lucide-react";
...
function HomePage() {
    ...
    return (<div ref={rootRef} className="min-h-screen bg-background text-foreground">
      {/* HEADER */}
      <header className={`fixed inset-x-0 top-0 z-50 bg-background transition-shadow duration-300 ${scrolled ? "shadow-sm border-b border-border" : ""}`}>
        <div className="container-x flex items-center justify-between h-20 gap-4">
          ...
          <nav className="hidden xl:flex items-center gap-7 text-sm font-medium">
            ...
            return (<a key={n.id} href={`#${n.id}`} className={`relative py-1 whitespace-nowrap transition-colors ${isActive ? "text-accent" : "text-foreground hover:text-accent"}`}>
                  {n.label}
                  <span className={`pointer-events-none absolute left-0 right-0 -bottom-0.5 h-0.5 rounded-full bg-accent origin-left transition-transform duration-300 ${isActive ? "scale-x-100" : "scale-x-0"}`}/>
                </a>);
        })}
            <Link to="/archiwum" className="relative py-1 transition-colors text-foreground hover:text-accent inline-flex items-center gap-1.5">
              <ArchiveIcon className="w-4 h-4"/>
              {t.nav.archive}
            </Link>
          </nav>
          <div className="flex items-center gap-2 sm:gap-3">
            <Link to="/archiwum" aria-label={t.nav.archive} className="xl:hidden inline-flex items-center justify-center w-10 h-10 rounded-full text-foreground hover:text-accent hover:bg-accent/10 transition-colors">
              <ArchiveIcon className="w-5 h-5"/>
            </Link>
            <LanguageSwitcher lang={lang} onChange={setLang} ariaLabel={t.nav.langAria}/>
            <Button asChild className="hidden sm:inline-flex bg-accent text-accent-foreground hover:bg-accent/90 rounded-full px-5">
              <a href="#rekrutacja">
      ...
      <section id="top" className="relative h-[100svh] min-h-[560px] sm:min-h-[640px] overflow-hidden">
        <div className="absolute inset-0">
          <img src={heroImg.url} alt="Kolegium" className="hidden sm:block w-full h-full object-cover object-center kenburns"/>
      ...
      <section id="oferta" className="py-24 md:py-32 bg-secondary/50 scroll-mt-24">
        <div className="container-x">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            return (<div key={f.title} className="reveal group bg-card rounded-2xl p-8 border border-border hover:border-accent/60 hover:shadow-xl transition-all">
      ...
      <section id="internat" className="py-24 md:py-32 container-x scroll-mt-24">
        <div className="grid lg:grid-cols-12 gap-10 lg:gap-16 items-center">
          <div className="lg:col-span-5">
            ...
            <ul className="reveal mt-8 space-y-3">
              {t.dorm.bullets.map((p) => (<li key={p} className="flex items-start gap-3">
        ...
        ].map((b) => (<div key={b.t} className="flex flex-col items-center text-center">
        ...
        <div className="mt-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
            {[dorm5, dorm6, dorm7, dorm8, dorm9, dorm10].map((img, i) => (<img key={i} src={img.url} alt="" className="reveal w-full aspect-square object-cover rounded-xl shadow hover:scale-[1.03] transition-transform"/>))}
          </div>
        </div>
      </section>
      ...
      <section id="kontakt" className="py-24 md:py-32 bg-secondary/50 scroll-mt-24">
        <div className="container-x grid lg:grid-cols-2 gap-12 items-start">
          ...
          <div className="reveal rounded-2xl overflow-hidden shadow-xl border border-border h-[480px]">
            <iframe title="Mapa — ul. Bobrowiecka 9, Warszawa" src="https://www.google.com/maps?q=ul.+Bobrowiecka+9,+Warszawa&output=embed" className="w-full h-full border-0" loading="lazy"/>
          </div>
        </div>
      </section>
        ...
        <div className="container-x grid gap-8 md:grid-cols-3 items-center">
          ...
          <div className="flex flex-col items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              {SOCIAL_LINKS.map((s) => (<a key={s.url} href={s.url} target="_blank" rel="noopener noreferrer" aria-label={s.label} className="w-10 h-10 rounded-full border border-primary-foreground/25 flex items-center justify-center hover:bg-accent hover:text-accent-foreground hover:border-accent transition-colors">
...
/* ---------- helper components ---------- */
function LanguageSwitcher({ lang, onChange, ariaLabel, }) {
    const current = languageOptions.find((o) => o.code === lang);
    return (<DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button type="button" className="inline-flex items-center gap-1.5 rounded-full border border-border bg-background px-3 py-1.5 text-sm font-medium hover:border-accent/60 transition-colors" aria-label={ariaLabel}>
      ...
      <DropdownMenuContent align="end" className="min-w-[180px]">
        {languageOptions.map((o) => (<DropdownMenuItem key={o.code} onClick={() => onChange(o.code)} className={`gap-2 cursor-pointer ${o.code === lang ? "font-semibold text-accent" : ""}`}>
...
function DocsCard({ title, subtitle, icon: Icon, items, highlight, downloadLabel, }) {
    return (<div className={`reveal flex flex-col rounded-2xl border p-7 transition-all hover:shadow-xl ${highlight ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border hover:border-accent/60"}`}>
      <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-5 ${highlight ? "bg-accent text-accent-foreground" : "bg-primary text-primary-foreground"}`}>
        <Icon className="w-7 h-7"/>
      </div>
      ...
      <ul className="space-y-2 flex-1">
        {items.map((item) => (<li key={item.name} className={`rounded-xl border p-3 ${highlight ? "border-primary-foreground/15 bg-primary-foreground/5" : "border-border bg-secondary/40"}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {item.files.map((f) => (<a key={f.kind + f.url} href={f.url} target="_blank" rel="noopener noreferrer" download className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-semibold tracking-wide transition-colors ${highlight ? "bg-accent text-accent-foreground hover:bg-accent/90" : "bg-primary text-primary-foreground hover:bg-primary/85"}`} aria-label={`${downloadLabel} ${item.name} (${f.kind})`}>
